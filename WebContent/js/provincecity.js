...
function CreateProvinceCity(provinceid, cityid) {
  var province = document.getElementById(provinceid.toString());
  var city = document.getElementById(cityid.toString());
  //生成省份
  province.length = 1;
  ...
  province.onchange = function() {
    showChild(province, city);
  }
}

function SetProvinceCityValue(provinceid, cityid, provincename, cityname) {
  var province = document.getElementById(provinceid.toString());
  var city = document.getElementById(cityid.toString());
  var province_option = province.options;
  var city_option = city.options;
  for (i = 0; i < province_option.length; i++) {
    if (province_option[i].value == provincename.toString()) {
      province_option[i].selected = true;
      break;
    }
  }
  showChild(province, city);
  for (i = 0; i < city_option.length; i++) {
    if (city_option[i].value == cityname.toString()) {
      city_option[i].selected = true;
      break;
    }
  }
}
...
//@childArr为子节点数组
//省份改变市
function showChild(current, child) {
  var currentValue = current.value;
  var count = cityArr.length;
  //每次切换市'把城市的子option长度设置为1,即清除城市的选择, DOM对象select元素是长度是子option的个数
  child.length = 1;
  for(var i = 0; i < count; i++) {
    //判断所选的值即父类，与当前节点第一个数组元素是否相同
    if(currentValue == cityArr[i][0]) {
      //不取第一个数组元素, 因为第一个是父类，所以j从1开始，而不是从0开始
